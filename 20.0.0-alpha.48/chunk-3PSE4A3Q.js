...
import { g as A } from "@nf-internal/chunk-M4JCPEPJ";
import { A as M, b as T, z as d } from "@nf-internal/chunk-STIBVO4O";
import * as a from "@angular/core";
import { InjectionToken as U, inject as f, DOCUMENT as J, ElementRef as K, ErrorHandler as Q, HostAttributeToken as X, booleanAttribute as Z } from "@angular/core";
import { take as tt } from "rxjs/operators";
import * as L from "@angular/common/http";
import { HttpClient as z } from "@angular/common/http";
import * as h from "@angular/core";
import { SecurityContext as g, DOCUMENT as O, Optional as F, SkipSelf as k, ErrorHandler as W } from "@angular/core";
import * as x from "@angular/platform-browser";
import { DomSanitizer as $ } from "@angular/platform-browser";
import { tap as w, map as _, catchError as j, finalize as V, share as B } from "rxjs/operators";
var S;
function Y() { if (S === void 0 && (S = null, typeof window < "u")) {
    let r = window;
    r.trustedTypes !== void 0 && (S = r.trustedTypes.createPolicy("angular#components", { createHTML: c => c }));
} return S; }
function p(r) { return Y()?.createHTML(r) || r; }
function R(r) { return Error(`Unable to find icon with the name "${r}"`); }
function P() { return Error("Could not find HttpClient for use with Angular Material icons. Please add provideHttpClient() to your providers."); }
...
var l = class {
    ...
    addSvgIconLiteralInNamespace(t, e, n, o) { let i = this._sanitizer.sanitize(g.HTML, n); if (!i)
    ...
    addSvgIconSetLiteralInNamespace(t, e, n) { let o = this._sanitizer.sanitize(g.HTML, e); if (!o)
    ...
    getSvgIconFromUrl(t) { let e = this._sanitizer.sanitize(g.RESOURCE_URL, t); if (!e)
        throw y(t); let n = this._cachedIconsByUrl.get(e); return n ? d(C(n)) : this._loadSvgIconFromConfig(new l(t, null)).pipe(w(o => this._cachedIconsByUrl.set(e, o)), _(o => C(o))); }
    getNamedSvgIcon(t, e = "") { let n = b(e, t), o = this._svgIconConfigs.get(n); if (o)
        return this._getSvgFromConfig(o); if (o = this._getIconConfigFromResolvers(e, t), o)
        return this._svgIconConfigs.set(n, o), this._getSvgFromConfig(o); let i = this._iconSetConfigs.get(e); return i ? this._getSvgFromIconSetConfigs(t, i) : M(R(n)); }
    ngOnDestroy() { this._resolvers = [], this._svgIconConfigs.clear(), this._iconSetConfigs.clear(), this._cachedIconsByUrl.clear(); }
    _getSvgFromConfig(t) { return t.svgText ? d(C(this._svgElementFromConfig(t))) : this._loadSvgIconFromConfig(t).pipe(_(e => C(e))); }
    _getSvgFromIconSetConfigs(t, e) { let n = this._extractIconWithNameFromAnySet(t, e); if (n)
        return d(n); let o = e.filter(i => !i.svgText).map(i => this._loadSvgIconSetFromConfig(i).pipe(j(s => { let m = `Loading icon set URL: ${this._sanitizer.sanitize(g.RESOURCE_URL, i.url)} failed: ${s.message}`; return this._errorHandler.handleError(new Error(m)), d(null); }))); return A(o).pipe(_(() => { let i = this._extractIconWithNameFromAnySet(t, e); if (!i)
        throw R(t); return i; })); }
    ...
    _loadSvgIconSetFromConfig(t) { return t.svgText ? d(null) : this._fetchIcon(t).pipe(w(e => t.svgText = e)); }
    ...
    _fetchIcon(t) { let { url: e, options: n } = t, o = n?.withCredentials ?? !1; if (!this._httpClient)
        throw P(); if (e == null)
        throw Error(`Cannot fetch icon from URL "${e}".`); let i = this._sanitizer.sanitize(g.RESOURCE_URL, e); if (!i)
        throw y(e); let s = this._inProgressUrlFetches.get(i); if (s)
        return s; let u = this._httpClient.get(i, { responseType: "text", withCredentials: o }).pipe(_(m => p(m)), V(() => this._inProgressUrlFetches.delete(i)), B()); return this._inProgressUrlFetches.set(i, u), u; }
    _addSvgIconConfig(t, e, n) { return this._svgIconConfigs.set(b(t, e), n), this; }
...
function b(r, c) { return r + ":" + c; }
function G(r) { return !!(r.url && r.options); }
import "@angular/common/http";
import "@angular/platform-browser";
import "@angular/cdk/a11y";
import "@angular/cdk/bidi";
var et = ["*"], nt = new U("MAT_ICON_DEFAULT_OPTIONS"), ot = new U("mat-icon-location", { providedIn: "root", factory: rt });
function rt() { let r = f(J), c = r ? r.location : null; return { getPathname: () => c ? c.pathname + c.search : "" }; }
var H = ["clip-path", "color-profile", "src", "cursor", "fill", "filter", "marker", "marker-start", "marker-mid", "marker-end", "mask", "stroke"], it = H.map(r => `[${r}]`).join(", "), st = /^url\(['"]?#(.*?)['"]?\)$/, Mt = (() => {
    class r {
        _elementRef = f(K);
        _iconRegistry = f(I);
        _location = f(ot);
        ...
        _currentIconFetch = T.EMPTY;
        constructor() { let t = f(new X("aria-hidden"), { optional: !0 }), e = f(nt, { optional: !0 }); e && (e.color && (this.color = this._defaultColor = e.color), e.fontSet && (this.fontSet = e.fontSet)), t || this._elementRef.nativeElement.setAttribute("aria-hidden", "true"); }
        ...
        _cacheChildrenWithExternalReferences(t) { let e = t.querySelectorAll(it), n = this._elementsWithExternalReferences = this._elementsWithExternalReferences || new Map; for (let o = 0; o < e.length; o++)
            H.forEach(i => { let s = e[o], u = s.getAttribute(i), m = u ? u.match(st) : null; if (m) {
                let v = n.get(s);
