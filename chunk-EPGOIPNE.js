...
import { b as Ot, c as Nt, d as Mt } from "@nf-internal/chunk-K25EXYVF";
import { a as h, b as w, d as ye, i as f, j as _t, k as It } from "@nf-internal/chunk-2AVGQ2MQ";
var G = (e, t) => { let r = []; if (e && r.push(e), t)
    for (let n of t)
        r.push(n); return r; }, z = (e, t) => `${e || "anonymous"}${t && t.length > 0 ? ` (a.k.a. ${t.join(",")})` : ""}`, X = () => { let e = [], t = [], r = !1, n = new Set, o = d => d.sort((l, m) => Dt[m.step] - Dt[l.step] || vt[m.priority || "normal"] - vt[l.priority || "normal"]), s = d => { let l = !1, m = g => { let E = G(g.name, g.aliases); if (E.includes(d)) {
    l = !0;
    for (let y of E)
        n.delete(y);
    return !1;
    ...
    y.relation === "after" && S.after.push(y), y.relation === "before" && S.before.push(y);
        ...
        if (S.some(x => n.has(x))) {
            ...
            for (let x of S) {
                let R = e.findIndex(_ => _.name === x || _.aliases?.some(v => v === x));
                if (R === -1)
                    continue;
                let b = e[R];
        ...
        if (S.some(x => n.has(x))) {
            if (!g)
                throw new Error(`Duplicate middleware name '${z(m, E)}'`);
            for (let x of S) {
                let R = t.findIndex(_ => _.name === x || _.aliases?.some(v => v === x));
                if (R === -1)
                    continue;
                let b = t[R];
        ...
        for (let x of S)
            n.add(x);
    } t.push(y); }, clone: () => a(X()), use: d => { d.applyToStack(p); }, remove: d => typeof d == "string" ? s(d) : i(d), removeByTag: d => { let l = !1, m = g => { let { tags: E, name: y, aliases: S } = g; if (E && E.includes(d)) {
        let x = G(y, S);
        for (let R of x)
            n.delete(R);
        return l = !0, !1;
    } return !0; }; return e = e.filter(m), t = t.filter(m), l; }, concat: d => { let l = a(X()); return l.use(d), l.identifyOnResolve(r || l.identifyOnResolve() || (d.identifyOnResolve?.() ?? !1)), l; }, applyToStack: a, identify: () => u(!0).map(d => { let l = d.step ?? d.relation + " " + d.toMiddleware; return z(d.name, d.aliases) + " - " + l; }), identifyOnResolve(d) { return typeof d == "boolean" && (r = d), r; }, resolve: (d, l) => { for (let m of u().map(g => g.middleware).reverse())
        d = m(d, l); return r && console.log(p.identify()), d; } }; return p; }, Dt = { initialize: 5, serialize: 4, build: 3, finalizeRequest: 2, deserialize: 1 }, vt = { high: 3, normal: 2, low: 1 };
var Pt = class {
    config;
    middlewareStack = X();
    initConfig;
...
var Rs = e => ({ setHttpHandler(t) { e.httpHandler = t; }, httpHandler() { return e.httpHandler; }, updateHttpClientConfig(t, r) { e.httpHandler?.updateHttpClientConfig(t, r); }, httpHandlerConfigs() { return e.httpHandler.httpHandlerConfigs(); } }), Ts = e => ({ httpHandler: e.httpHandler() });
var A = class e {
    method;
    protocol;
    hostname;
    ...
    static clone(t) { let r = new e(w(h({}, t), { headers: h({}, t.headers) })); return r.query && (r.query = jn(r.query)), r; }
...
function jn(e) { return Object.keys(e).reduce((t, r) => { let n = e[r]; return w(h({}, t), { [r]: Array.isArray(n) ? [...n] : n }); }, {}); }
var I = class {
    statusCode;
    reason;
    headers;
  ...
  ` + l;
            ...
            try {
                if (I.isInstance(o)) {
                    ...
                    d.$metadata = { httpStatusCode: o.statusCode, requestId: qe(/^x-[\w-]+-request-?id$/, g), extendedRequestId: qe(/^x-[\w-]+-id-2$/, g), cfId: qe(/^x-[\w-]+-cf-id$/, g) };
                }
            }
            catch { }
        }
...
}), qe = (e, t) => (t.find(([r]) => r.match(e)) || [void 0, void 0])[1];
...
var qn = { name: "deserializerMiddleware", step: "deserialize", tags: ["DESERIALIZER"], override: !0 }, Gn = { name: "serializerMiddleware", step: "serialize", tags: ["SERIALIZER"], override: !0 };
function js(e) { return { applyToStack: t => { t.add(kt(e), Gn), t.add($t(e), qn), e.protocol.setSerdeContext(e); } }; }
...
var M = (() => { class e {
    ...
    memberTraits;
    normalizedTraits;
    constructor(r, n) { this.ref = r, this.memberName = n; let o = [], s = r, i = r; for (this._isMemberSchema = !1; Ge(s);)
        ...
        return;
    } if (this.schema = Ee(i), Vn(this.schema) ? (this.name = `${this.schema[1]}#${this.schema[2]}`, this.traits = this.schema[3]) : (this.name = this.memberName ?? String(i), this.traits = 0), this._isMemberSchema && !n)
    ...
    isStructSchema() { let n = this.getSchema()[0]; return n === 3 || n === -3 || n === 4; }
    isUnionSchema() { return this.getSchema()[0] === 4; }
...
function se(e, t) { if (e instanceof M)
    return Object.assign(e, { memberName: t, _isMemberSchema: !0 }); let r = M; return new r(e, t); }
var Ge = e => Array.isArray(e) && e.length === 2, Vn = e => Array.isArray(e) && e.length >= 5;
var ie = class e {
    namespace;
    schemas;
    exceptions;
...
var Ve = "***SensitiveInformation***";
function xe(e, t) { if (t == null)
    return t; let r = M.of(e); if (r.getMergedTraits().sensitive)
    return Ve; if (r.isListSchema()) {
...
var We = class {
    middlewareStack = X();
    schema;
    static classBuilder() { return new Ye; }
    ...
    build() { let t = this, r; return r = class extends We {
        input;
...
var Wn = e => t => r => f(null, null, function* () { if (!A.isInstance(r.request))
...
} return t(r); }), Yn = { name: "hostHeaderMiddleware", step: "build", priority: "low", tags: ["HOST"], override: !0 }, ii = e => ({ applyToStack: t => { t.add(Wn(e), Yn); } });
var Kn = () => (e, t) => r => f(null, null, function* () { try {
...
} }), Xn = { name: "loggerMiddleware", tags: ["LOGGER"], step: "initialize", override: !0 }, ui = e => ({ applyToStack: t => { t.add(Kn(), Xn); } });
...
var yi = e => ({ applyToStack: t => { t.add(Lt(), Ut); } });
...
function Qn(e) { let t = new Map; for (let r of e)
    t.set(r.schemeId, r); return t; }
var Ft = (e, t) => (r, n) => o => f(null, null, function* () {
    let s = e.httpAuthSchemeProvider(yield t.httpAuthSchemeParametersProvider(e, n, o.input)), i = e.authSchemePreference ? yield e.authSchemePreference() : [], a = Ht(s, i), c = Qn(e.httpAuthSchemes), u = P(n), p = [];
    for (let d of a) {
...
var Jn = { step: "serialize", tags: ["HTTP_AUTH_SCHEME"], name: "httpAuthSchemeMiddleware", override: !0, relation: "before", toMiddleware: "endpointV2Middleware" }, bi = (e, { httpAuthSchemeParametersProvider: t, identityProviderConfigProvider: r }) => ({ applyToStack: n => { n.addRelativeTo(Ft(e, { httpAuthSchemeParametersProvider: t, identityProviderConfigProvider: r }), Jn); } });
var Zn = e => t => { throw t; }, eo = (e, t) => { }, zt = e => (t, r) => n => f(null, null, function* () { if (!A.isInstance(n.request))
    return t(n); let s = P(r).selectedHttpAuthScheme; if (!s)
    throw new Error("No HttpAuthScheme was selected: unable to sign request"); let { httpAuthOption: { signingProperties: i = {} }, identity: a, signer: c } = s, u = yield t(w(h({}, n), { request: yield c.sign(n.request, a, i) })).catch((c.errorHandler || Zn)(i)); return (c.successHandler || eo)(u.response, i), u; });
var to = { step: "finalizeRequest", tags: ["HTTP_SIGNING"], name: "httpSigningMiddleware", aliases: ["apiKeyMiddleware", "tokenMiddleware", "awsAuthMiddleware"], override: !0, relation: "after", toMiddleware: "retryMiddleware" }, Di = e => ({ applyToStack: t => { t.addRelativeTo(zt(e), to); } });
var ro = (e, t, r, n = s => s, ...o) => f(null, null, function* () { let s = new e(r); return s = n(s) ?? s, yield t.send(s, ...o); });
function no(e, t, r, n, o) { return function (i, a, ...c) { return It(this, null, function* () { let u = a, p = i.startingToken ?? u[r], d = !0, l; for (; d;) {
    if (u[r] = p, o && (u[o] = u[o] ?? i.pageSize), i.client instanceof e)
        l = yield new _t(ro(t, i.client, a, i.withCommand, ...c));
    else
        throw new Error(`Invalid client, expected instance of ${e.name}`);
    yield l;
    let m = p;
    p = oo(l, n), d = !!(p && (!i.stopOnSameToken || p !== m));
} return void 0; }); }; }
var oo = (e, t) => { let r = e, n = t.split("."); for (let o of n) {
...
var L = e => { if (typeof e == "function")
    return e; let t = Promise.resolve(e); return () => t; };
var qt = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", Ke = Object.entries(qt).reduce((e, [t, r]) => (e[r] = Number(t), e), {}), Gt = qt.split(""), B = 6, W = 8, Vt = 63;
...
var k = e => encodeURIComponent(e).replace(/[!'()*]/g, so), so = e => `%${e.charCodeAt(0).toString(16).toUpperCase()}`;
...
function Xe(e, t) { return new Request(e, t); }
function Kt(e = 0) { return new Promise((t, r) => { e && setTimeout(() => { let n = new Error(`Request did not complete within ${e} ms`); n.name = "TimeoutError", r(n); }, e); }); }
var Qe = { supported: void 0 }, Xt = class e {
    config;
    configProvider;
    static create(t) { return typeof t?.handle == "function" ? t : new e(t); }
    constructor(t) { typeof t == "function" ? this.configProvider = t().then(r => r || {}) : (this.config = t ?? {}, this.configProvider = Promise.resolve(this.config)), Qe.supported === void 0 && (Qe.supported = typeof Request < "u" && "keepalive" in Xe("https://[::1]")); }
    destroy() { }
...
var ca = e => f(null, null, function* () { return typeof Blob == "function" && e instanceof Blob || e.constructor?.name === "Blob" ? Blob.prototype.arrayBuffer !== void 0 ? new Uint8Array(yield e.arrayBuffer()) : io(e) : ao(e); });
function io(e) { return f(this, null, function* () { let t = yield co(e), r = Re(t); return new Uint8Array(r); }); }
function ao(e) { return f(this, null, function* () { let t = [], r = e.getReader(), n = !1, o = 0; for (; !n;) {
...
function co(e) { return new Promise((t, r) => { let n = new FileReader; n.onloadend = () => { if (n.readyState !== 2)
...
for (let e = 0; e < 256; e++) {
    ...
    t.length === 1 && (t = `0${t}`), Qt[e] = t, Je[t] = e;
}
...
function O(e) { let t = ""; for (let r = 0; r < e.byteLength; r++)
    t += Qt[e[r]]; return t; }
var Ze = (...r) => f(null, [...r], function* (e = new Uint8Array, t) { if (e instanceof Uint8Array)
...
var et = e => { if (e != null) {
    if (typeof e == "string") {
        let t = parseFloat(e);
        if (!Number.isNaN(t))
            return String(t) !== String(e) && ho.warn(mo(`Expected number but observed string: ${e}`)), t;
    }
    if (typeof e == "number")
        return e;
    throw new TypeError(`Expected number, got ${typeof e}: ${e}`);
} }, uo = Math.ceil(34028234663852886e22), Zt = e => { let t = et(e); if (t !== void 0 && !Number.isNaN(t) && t !== 1 / 0 && t !== -1 / 0 && Math.abs(t) > uo)
    throw new TypeError(`Expected 32-bit float, got ${e}`); return t; }, fo = e => { if (e != null) {
    if (Number.isInteger(e) && !Number.isNaN(e))
        return e;
    throw new TypeError(`Expected integer, got ${typeof e}: ${e}`);
} };
var er = e => rr(e, 16), tr = e => rr(e, 8), rr = (e, t) => { let r = fo(e); if (r !== void 0 && lo(r, t) !== r)
    throw new TypeError(`Expected ${t}-bit integer, got ${e}`); return r; }, lo = (e, t) => { switch (t) {
    case 32: return Int32Array.of(e)[0];
    case 16: return Int16Array.of(e)[0];
    case 8: return Int8Array.of(e)[0];
} };
var nr = e => et(typeof e == "string" ? Te(e) : e);
var or = e => Zt(typeof e == "string" ? Te(e) : e), po = /(-?(?:0|[1-9]\d*)(?:\.\d+)?(?:[eE][+-]?\d+)?)|(-?Infinity)|(NaN)/g, Te = e => { let t = e.match(po); if (t === null || t[0].length !== e.length)
    throw new TypeError("Expected real number, got implicit NaN"); return parseFloat(e); };
var ae = e => er(typeof e == "string" ? Te(e) : e), sr = e => tr(typeof e == "string" ? Te(e) : e), mo = e => String(new TypeError(e).stack || e).split(`
`).slice(0, 5).filter(t => !t.includes("stackTraceWarning")).join(`
`), ho = { warn: console.warn };
var go = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"], rt = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
function ya(e) { let t = e.getUTCFullYear(), r = e.getUTCMonth(), n = e.getUTCDay(), o = e.getUTCDate(), s = e.getUTCHours(), i = e.getUTCMinutes(), a = e.getUTCSeconds(), c = o < 10 ? `0${o}` : `${o}`, u = s < 10 ? `0${s}` : `${s}`, p = i < 10 ? `0${i}` : `${i}`, d = a < 10 ? `0${a}` : `${a}`; return `${go[n]}, ${c} ${rt[r]} ${t} ${u}:${p}:${d} GMT`; }
var Sa = new RegExp(/^(\d{4})-(\d{2})-(\d{2})[tT](\d{2}):(\d{2}):(\d{2})(?:\.(\d+))?[zZ]$/);
var yo = new RegExp(/^(\d{4})-(\d{2})-(\d{2})[tT](\d{2}):(\d{2}):(\d{2})(?:\.(\d+))?(([-+]\d{2}\:\d{2})|[zZ])$/), Ea = e => { if (e == null)
    return; if (typeof e != "string")
    throw new TypeError("RFC-3339 date-times must be expressed as strings"); let t = yo.exec(e); if (!t)
    throw new TypeError("Invalid RFC-3339 date-time value"); let [r, n, o, s, i, a, c, u, p] = t, d = ae(ce(n)), l = j(o, "month", 1, 12), m = j(s, "day", 1, 31), g = Ae(d, l, m, { hours: i, minutes: a, seconds: c, fractionalMilliseconds: u }); return p.toUpperCase() != "Z" && g.setTime(g.getTime() - Io(p)), g; }, So = new RegExp(/^(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun), (\d{2}) (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) (\d{4}) (\d{1,2}):(\d{2}):(\d{2})(?:\.(\d+))? GMT$/), Eo = new RegExp(/^(?:Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday), (\d{2})-(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)-(\d{2}) (\d{1,2}):(\d{2}):(\d{2})(?:\.(\d+))? GMT$/), wo = new RegExp(/^(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun) (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) ( [1-9]|\d{2}) (\d{1,2}):(\d{2}):(\d{2})(?:\.(\d+))? (\d{4})$/), wa = e => { if (e == null)
    return; if (typeof e != "string")
    throw new TypeError("RFC-7231 date-times must be expressed as strings"); let t = So.exec(e); if (t) {
    let [r, n, o, s, i, a, c, u] = t;
    return Ae(ae(ce(s)), tt(o), j(n, "day", 1, 31), { hours: i, minutes: a, seconds: c, fractionalMilliseconds: u });
} if (t = Eo.exec(e), t) {
    let [r, n, o, s, i, a, c, u] = t;
    return To(Ae(xo(s), tt(o), j(n, "day", 1, 31), { hours: i, minutes: a, seconds: c, fractionalMilliseconds: u }));
} if (t = wo.exec(e), t) {
    let [r, n, o, s, i, a, c, u] = t;
    return Ae(ae(ce(u)), tt(n), j(o.trimLeft(), "day", 1, 31), { hours: s, minutes: i, seconds: a, fractionalMilliseconds: c });
} throw new TypeError("Invalid RFC-7231 date-time value"); }, xa = e => { if (e == null)
    ...
    throw new TypeError("Epoch timestamps must be valid, non-Infinite, non-NaN numerics"); return new Date(Math.round(t * 1e3)); }, Ae = (e, t, r, n) => { let o = t - 1; return bo(e, o, r), new Date(Date.UTC(e, o, r, j(n.hours, "hour", 0, 23), j(n.minutes, "minute", 0, 59), j(n.seconds, "seconds", 0, 60), _o(n.fractionalMilliseconds))); }, xo = e => { let t = new Date().getUTCFullYear(), r = Math.floor(t / 100) * 100 + ae(ce(e)); return r < t ? r + 100 : r; }, Ro = 50 * 365 * 24 * 60 * 60 * 1e3, To = e => e.getTime() - new Date().getTime() > Ro ? new Date(Date.UTC(e.getUTCFullYear() - 100, e.getUTCMonth(), e.getUTCDate(), e.getUTCHours(), e.getUTCMinutes(), e.getUTCSeconds(), e.getUTCMilliseconds())) : e, tt = e => { let t = rt.indexOf(e); if (t < 0)
    throw new TypeError(`Invalid month: ${e}`); return t + 1; }, Ao = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31], bo = (e, t, r) => { let n = Ao[t]; if (t === 1 && Co(e) && (n = 29), r > n)
    throw new TypeError(`Invalid day for ${rt[t]} in ${e}: ${r}`); }, Co = e => e % 4 === 0 && (e % 100 !== 0 || e % 400 === 0), j = (e, t, r, n) => { let o = sr(ce(e)); if (o < r || o > n)
    throw new TypeError(`${t} must be between ${r} and ${n}, inclusive`); return o; }, _o = e => e == null ? 0 : or("0." + e) * 1e3, Io = e => { let t = e[0], r = 1; if (t == "+")
    r = 1;
else if (t == "-")
    r = -1;
else
...
var C = Array.from({ length: 256 }, (e, t) => t.toString(16).padStart(2, "0")), ot = () => { if (nt)
    return nt(); let e = new Uint8Array(16); return crypto.getRandomValues(e), e[6] = e[6] & 15 | 64, e[8] = e[8] & 63 | 128, C[e[0]] + C[e[1]] + C[e[2]] + C[e[3]] + "-" + C[e[4]] + C[e[5]] + "-" + C[e[6]] + C[e[7]] + "-" + C[e[8]] + C[e[9]] + "-" + C[e[10]] + C[e[11]] + C[e[12]] + C[e[13]] + C[e[14]] + C[e[15]]; };
...
var cr = class extends be {
    ...
    setHostPrefix(t, r, n) { let o = M.of(r.input), s = V(r.traits ?? {}); if (s.endpoint) {
        let i = s.endpoint?.[0];
        if (typeof i == "string") {
...
function ka(e, t) { if (t.timestampFormat.useTrait && e.isTimestampSchema() && (e.getSchema() === 5 || e.getSchema() === 6 || e.getSchema() === 7))
...
var Oo = e => function (r) { return st(r) && r.expiration.getTime() - Date.now() < e; };
var fr = Oo(3e5), st = e => e.expiration !== void 0, lr = (e, t, r) => { if (e === void 0)
...
var No = void 0;
function Mo(e) { return e === void 0 ? !0 : typeof e == "string" && e.length <= 50; }
function Za(e) { let t = L(e.userAgentAppId ?? No), { customUserAgent: r } = e; return Object.assign(e, { customUserAgent: typeof r == "string" ? [[r]] : r, userAgentAppId: () => f(null, null, function* () { let n = yield t(); if (!Mo(n)) {
...
}, it = (e, t = {}) => { Object.entries(t).filter(([, n]) => n !== void 0).forEach(([n, o]) => { (e[n] == null || e[n] === "") && (e[n] = o); }); let r = e.message || e.Message || "UnknownError"; return e.message = r, delete e.Message, e; };
var mr = class {
    capacity;
    data = new Map;
    parameters = [];
...
var Do = new RegExp("^(?:25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]\\d|\\d)(?:\\.(?:25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]\\d|\\d)){3}$"), ue = e => Do.test(e) || e.startsWith("[") && e.endsWith("]");
var vo = new RegExp("^(?!.*-$)(?!-)[a-zA-Z0-9-]{1,63}$"), Y = (e, t = !1) => { if (!t)
    return vo.test(e); let r = e.split("."); for (let n of r)
    if (!Y(n))
        return !1; return !0; };
var de = {};
var F = "endpoints";
...
var yr = e => e != null;
var Sr = e => !e;
var at = { [re.HTTP]: 80, [re.HTTPS]: 443 }, Er = e => { let t = (() => { try {
    if (e instanceof URL)
        return e;
    ...
    return null; let u = ue(o), p = r.includes(`${n}:${at[c]}`) || typeof e == "string" && e.includes(`${n}:${at[c]}`), d = `${n}${p ? `:${at[c]}` : ""}`; return { scheme: c, authority: d, path: s, normalizedPath: s.endsWith("/") ? s : `${s}/`, isIp: u }; };
var wr = (e, t) => e === t;
...
var K = (e, t, r) => { if (typeof e == "string")
    return _e(e, r); if (e.fn)
    return br.callFunction(e, r); if (e.ref)
    return Ar(e, r); throw new T(`'${t}': ${String(e)} is not a string, function or reference.`); }, Ie = ({ fn: e, argv: t }, r) => { let n = t.map(s => ["boolean", "number"].includes(typeof s) ? s : br.evaluateExpression(s, "arg", r)), o = e.split("."); return o[0] in de && o[1] != null ? de[o[0]][o[1]](...n) : Tr[e](...n); }, br = { evaluateExpression: K, callFunction: Ie };
var Cr = (n, r) => { var o = n, { assign: e } = o, t = ye(o, ["assign"]); if (e && e in r.referenceRecord)
    throw new T(`'${e}' is already defined in Reference Record.`); let s = Ie(t, r); return r.logger?.debug?.(`${F} evaluateCondition: ${N(t)} = ${N(s)}`), h({ result: s === "" ? !0 : !!s }, e != null && { toAssign: { name: e, value: s } }); };
var Z = (e = [], t) => { let r = {}; for (let n of e) {
    let { result: o, toAssign: s } = Cr(n, w(h({}, t), { referenceRecord: h(h({}, t.referenceRecord), r) }));
    if (!o)
        return { result: o };
    s && (r[s.name] = s.value, t.logger?.debug?.(`${F} assign: ${s.name} := ${N(s.value)}`));
} return { result: !0, referenceRecord: r }; };
var _r = (e, t) => Object.entries(e).reduce((r, [n, o]) => w(h({}, r), { [n]: o.map(s => { let i = K(s, "Header value entry", t); if (typeof i != "string")
        throw new T(`Header '${n}' value '${i}' is not a string`); return i; }) }), {});
var ct = (e, t) => Object.entries(e).reduce((r, [n, o]) => w(h({}, r), { [n]: Or.getEndpointProperty(o, t) }), {}), Ir = (e, t) => { if (Array.isArray(e))
    return e.map(r => Ir(r, t)); switch (typeof e) {
    case "string": return _e(e, t);
    case "object":
        if (e === null)
            throw new T(`Unexpected endpoint property: ${e}`);
        return Or.getEndpointProperties(e, t);
    case "boolean": return e;
    default: throw new T(`Unexpected endpoint property type: ${typeof e}`);
} }, Or = { getEndpointProperty: Ir, getEndpointProperties: ct };
var Nr = (e, t) => { let r = K(e, "Endpoint URL", t); if (typeof r == "string")
    try {
        return new URL(r);
    }
    catch (n) {
...
var Mr = (e, t) => { let { conditions: r, endpoint: n } = e, { result: o, referenceRecord: s } = Z(r, t); if (!o)
    return; let i = w(h({}, t), { referenceRecord: h(h({}, t.referenceRecord), s) }), { url: a, properties: c, headers: u } = n; return t.logger?.debug?.(`${F} Resolving endpoint from template: ${N(n)}`), w(h(h({}, u != null && { headers: _r(u, i) }), c != null && { properties: ct(c, i) }), { url: Nr(a, i) }); };
var Dr = (e, t) => { let { conditions: r, error: n } = e, { result: o, referenceRecord: s } = Z(r, t); if (o)
    throw new T(K(n, "Error", w(h({}, t), { referenceRecord: h(h({}, t.referenceRecord), s) }))); };
var ut = (e, t) => { for (let r of e)
    if (r.type === "endpoint") {
        let n = Mr(r, t);
        if (n)
            return n;
        ...
        let n = vr.evaluateTreeRule(r, t);
        if (n)
            return n;
    }
    else
        throw new T(`Unknown endpoint rule: ${r}`); throw new T("Rules evaluation failed"); }, Po = (e, t) => { let { conditions: r, rules: n } = e, { result: o, referenceRecord: s } = Z(r, t); if (o)
    return vr.evaluateRules(n, w(h({}, t), { referenceRecord: h(h({}, t.referenceRecord), s) })); }, vr = { evaluateRules: ut, evaluateTreeRule: Po };
var Ou = (e, t) => { let { endpointParams: r, logger: n } = t, { parameters: o, rules: s } = e; t.logger?.debug?.(`${F} Initial EndpointParams: ${N(r)}`); let i = Object.entries(o).filter(([, u]) => u.default != null).map(([u, p]) => [u, p.default]); if (i.length > 0)
    for (let [u, p] of i)
        r[u] = r[u] ?? p; let a = Object.entries(o).filter(([, u]) => u.required).map(([u]) => u); for (let u of a)
    if (r[u] == null)
        throw new T(`Missing required parameter: '${u}'`); let c = ut(s, { endpointParams: r, logger: n, referenceRecord: {} }); return t.logger?.debug?.(`${F} Resolved endpoint: ${N(c)}`), c; };
var dt = (e, t = !1) => { if (t) {
...
var ko = $r, Uo = "", kr = e => { let { partitions: t } = ko; for (let n of t) {
...
var Ur = () => Uo;
var Lo = { isVirtualHostableS3Bucket: dt, parseArn: Pr, partition: kr };
de.aws = Lo;
...
var ft = e => I.isInstance(e) ? e.headers?.date ?? e.headers?.Date : void 0;
var ee = e => new Date(Date.now() + e);
var Hr = (e, t) => Math.abs(ee(t).getTime() - e) >= 3e5;
var lt = (e, t) => { let r = Date.parse(e); return Hr(r, t) ? r - Date.now() : t; };
var fe = (e, t) => { if (!t)
    ...
    errorHandler(t) { return r => { let n = r.ServerTime ?? ft(r.$response); if (n) {
        let o = fe("config", t.config), s = o.systemClockOffset;
        o.systemClockOffset = lt(n, o.systemClockOffset), o.systemClockOffset !== s && r.$metadata && (r.$metadata.clockSkewCorrected = !0);
    } throw r; }; }
    successHandler(t, r) { let n = ft(t); if (n) {
        let o = fe("config", r.config);
        o.systemClockOffset = lt(n, o.systemClockOffset);
    } }
};
...
var Fr = (e, t, r) => { let n, o, s, i = !1, a = () => f(null, null, function* () { o || (o = e()); try {
    n = yield o, s = !0, i = !1;
}
finally {
    o = void 0;
...
var Md = e => (e.sigv4aSigningRegionSet = L(e.sigv4aSigningRegionSet), e);
var zr = "X-Amz-Algorithm", Br = "X-Amz-Credential", mt = "X-Amz-Date", jr = "X-Amz-SignedHeaders", qr = "X-Amz-Expires", ht = "X-Amz-Signature", gt = "X-Amz-Security-Token";
var yt = "authorization", St = mt.toLowerCase(), Ho = "date", Gr = [yt, St, Ho], Vr = ht.toLowerCase(), pe = "x-amz-content-sha256", Wr = gt.toLowerCase();
var Yr = { authorization: !0, "cache-control": !0, connection: !0, expect: !0, from: !0, "keep-alive": !0, "max-forwards": !0, pragma: !0, referer: !0, te: !0, trailer: !0, "transfer-encoding": !0, upgrade: !0, "user-agent": !0, "x-amzn-trace-id": !0 }, Kr = /^proxy-/, Xr = /^sec-/;
var Ne = "AWS4-HMAC-SHA256";
var Qr = "AWS4-HMAC-SHA256-PAYLOAD", Jr = "UNSIGNED-PAYLOAD", Zr = 50, Et = "aws4_request", en = 3600 * 24 * 7;
var Me = {}, wt = [], De = (e, t, r) => `${e}/${t}/${r}/${Et}`, rn = (e, t, r, n, o) => f(null, null, function* () { let s = yield tn(e, t.secretAccessKey, t.accessKeyId), i = `${r}:${n}:${o}:${O(s)}:${t.sessionToken}`; if (i in Me)
    return Me[i]; for (wt.push(i); wt.length > Zr;)
    delete Me[wt.shift()]; let a = `AWS4${t.secretAccessKey}`; for (let c of [r, n, o, Et])
    a = yield tn(e, a, c); return Me[i] = a; });
var tn = (e, t, r) => { let n = new e(t); return n.update(H(r)), n.digest(); };
var xt = ({ headers: e }, t, r) => { let n = {}; for (let o of Object.keys(e).sort()) {
    if (e[o] == null)
        continue;
    let s = o.toLowerCase();
    (s in Yr || t?.has(s) || Kr.test(s) || Xr.test(s)) && (!r || r && !r.has(s)) || (n[s] = e[o].trim().replace(/\s+/g, " "));
} return n; };
var nn = e => typeof ArrayBuffer == "function" && e instanceof ArrayBuffer || Object.prototype.toString.call(e) === "[object ArrayBuffer]";
var ve = (n, o) => f(null, [n, o], function* ({ headers: e, body: t }, r) { for (let s of Object.keys(e))
    if (s.toLowerCase() === pe)
    ...
    let s = new r;
    return s.update(H(t)), O(yield s.digest());
} return Jr; });
var Pe = class {
    ...
    formatHeaderValue(t) { switch (t.type) {
        ...
        case "timestamp":
            ...
            return p[0] = 8, p.set(Rt.fromNumber(t.value.valueOf()).bytes, 1), p;
        case "uuid":
            if (!Fo.test(t.value))
...
var Fo = /^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/, Rt = class e {
    bytes;
    ...
    valueOf() { let t = this.bytes.slice(0), r = t[0] & 128; return r && on(t), parseInt(O(t), 16) * (r ? -1 : 1); }
...
var Tt = e => { e = A.clone(e); for (let t of Object.keys(e.headers))
...
var un = e => zo(e).toISOString().replace(/\.\d{3}Z$/, "Z"), zo = e => typeof e == "number" ? new Date(e * 1e3) : typeof e == "string" ? Number(e) ? new Date(Number(e) * 1e3) : new Date(e) : e;
var $e = class {
    service;
    regionProvider;
    credentialProvider;
...
var me = class extends $e {
    ...
    presign(n) { return f(this, arguments, function* (t, r = {}) { let { signingDate: o = new Date, expiresIn: s = 3600, unsignableHeaders: i, unhoistableHeaders: a, signableHeaders: c, hoistableHeaders: u, signingRegion: p, signingService: d } = r, l = yield this.credentialProvider(); this.validateResolvedCredentials(l); let m = p ?? (yield this.regionProvider()), { longDate: g, shortDate: E } = this.formatDate(o); if (s > en)
        return Promise.reject("Signature version 4 presigned URLs must have an expiration date less than one week in the future"); let y = De(E, m, d ?? this.service), S = an(Tt(t), { unhoistableHeaders: a, hoistableHeaders: u }); l.sessionToken && (S.query[gt] = l.sessionToken), S.query[zr] = Ne, S.query[Br] = `${l.accessKeyId}/${y}`, S.query[mt] = g, S.query[qr] = s.toString(10); let x = xt(S, i, c); return S.query[jr] = this.getCanonicalHeaderList(x), S.query[ht] = yield this.getSignature(g, y, this.getSigningKey(l, m, E, d), this.createCanonicalRequest(S, x, yield ve(t, this.sha256))), S; }); }
    ...
    signEvent(a, c) {
        return f(this, arguments, function* ({ headers: t, payload: r }, { signingDate: n = new Date, priorSignature: o, signingRegion: s, signingService: i }) {
            let u = s ?? (yield this.regionProvider()), { shortDate: p, longDate: d } = this.formatDate(n), l = De(p, u, i ?? this.service), m = yield ve({ headers: {}, body: r }, this.sha256), g = new this.sha256;
            g.update(t);
            let E = O(yield g.digest()), y = [Qr, d, l, o, E, m].join(`
`);
...
var fn = e => { let t = []; for (let r in ne) {
    let n = ne[r];
...
var el = e => Object.assign(fn(e), pn(e));
var tl = e => Object.assign(ln(e), mn(e));
var ke = class {
    trace() { }
    debug() { }
    info() { }
...
var hn = class {
    ...
    decorateServiceException(t, r = {}) { if (this.queryCompat) {
        let n = t.Message ?? r.Message, o = it(t, r);
    ...
    findQueryCompatibleError(t, r) { try {
        ...
        return t.find(o => M.of(o).getMergedTraits().awsQueryError?.[0] === r);
    } }
};
var gn = class {
    serdeContext;
...
var yn = class {
    ...
    to;
    keys;
    constructor(t, r) { this.from = t, this.to = r, this.keys = new Set(Object.keys(this.from).filter(n => n !== "__type")); }
    mark(t) { this.keys.delete(t); }
    hasUnknown() { return this.keys.size === 1 && Object.keys(this.to).length === 0; }
    writeUnknown() { if (this.hasUnknown()) {
        let t = this.keys.values().next().value, r = this.from[t];
        this.to.$unknown = [t, r];
    } }
};
var gl = (e, t) => Ze(e, t).then(r => (t?.utf8Encoder ?? Se)(r));
var qo = /\d{12}\.ddb/;
function Sn(e, t, r) { return f(this, null, function* () { if (r.request?.headers?.["smithy-protocol"] === "rpc-v2-cbor" && D(e, "PROTOCOL_RPC_V2_CBOR", "M"), typeof t.retryStrategy == "function") {
    ...
    let s = e.endpointV2;
    switch (String(s?.url?.hostname).match(qo) && D(e, "ACCOUNT_ID_ENDPOINT", "O"), yield t.accountIdEndpointMode?.()) {
        case "disabled":
...
var At = "user-agent", Ue = "x-amz-user-agent";
...
var Go = e => (t, r) => n => f(null, null, function* () { let { request: o } = n; if (!A.isInstance(o))
    return t(n); let { headers: s } = o, i = r?.userAgent?.map(Le) || [], a = (yield e.defaultUserAgentProvider()).map(Le); yield Sn(r, e, n); let c = r; a.push(`m/${xn(Object.assign({}, r.__smithy_context?.features, c.__aws_sdk_context?.features))}`); let u = e?.customUserAgent?.map(Le) || [], p = yield e.userAgentAppId(); p && a.push(Le(["app", `${p}`])); let d = Ur(), l = (d ? [d] : []).concat([...a, ...i, ...u]).join(" "), m = [...a.filter(g => g.startsWith("aws-sdk-")), ...u].join(" "); return e.runtime !== "browser" ? (m && (s[Ue] = s[Ue] ? `${s[At]} ${m}` : m), s[At] = l) : s[Ue] = l, t(w(h({}, n), { request: o })); }), Le = e => { let t = e[0].split("/").map(i => i.replace(En, "-")).join("/"), r = e[1]?.replace(wn, "-"), n = t.indexOf("/"), o = t.substring(0, n), s = t.substring(n + 1); return o === "api" && (s = s.toLowerCase()), [o, s, r].filter(i => i && i.length > 0).reduce((i, a, c) => { switch (c) {
    case 0: return a;
    case 1: return `${i}/${a}`;
    default: return `${i}#${a}`;
} }, ""); }, Vo = { name: "getUserAgentMiddleware", step: "build", priority: "low", tags: ["SET_USER_AGENT", "USER_AGENT"], override: !0 }, Il = e => ({ applyToStack: t => { t.add(Go(e), Vo); } });
var An = new Set, bn = (e, t = Y) => { if (!An.has(e) && !t(e))
    ...
    An.add(e); };
var He = e => typeof e == "string" && (e.startsWith("fips-") || e.endsWith("-fips"));
var Cn = e => He(e) ? ["fips-aws-global", "aws-fips"].includes(e) ? "us-east-1" : e.replace(/fips-(dkr-|prod-)?|-fips/, "") : e;
var Hl = e => { let { region: t, useFipsEndpoint: r } = e; if (!t)
    throw new Error("Region is missing"); return Object.assign(e, { region: () => f(null, null, function* () { let n = typeof t == "function" ? yield t() : t, o = Cn(n); return bn(o), o; }), useFipsEndpoint: () => f(null, null, function* () { let n = typeof t == "string" ? t : yield t(); return He(n) ? !0 : typeof r != "function" ? Promise.resolve(!!r) : r(); }) }); };
var _n = "content-length";
function Wo(e) { return t => r => f(null, null, function* () { let n = r.request; if (A.isInstance(n)) {
...
var Yo = { step: "build", tags: ["SET_CONTENT_LENGTH", "CONTENT_LENGTH"], name: "contentLengthMiddleware", override: !0 }, jl = e => ({ applyToStack: t => { t.add(Wo(e.bodyLengthChecker), Yo); } });
var In = ({ config: e, instructions: t }) => (r, n) => o => f(null, null, function* () { e.isCustomEndpoint && ur(n, "ENDPOINT_OVERRIDE", "N"); let s = yield Mt(o.input, { getEndpointParameterInstructions() { return t; } }, h({}, e), n); n.endpointV2 = s, n.authSchemes = s.properties?.authSchemes; let i = n.authSchemes?.[0]; if (i) {
...
var Ko = { step: "serialize", tags: ["ENDPOINT_PARAMETERS", "ENDPOINT_V2", "ENDPOINT"], name: "endpointV2Middleware", override: !0, relation: "before", toMiddleware: jt.name }, Zl = (e, t) => ({ applyToStack: r => { r.addRelativeTo(In({ config: e, instructions: t }), Ko); } });
var op = e => { let t = e.tls ?? !0, { endpoint: r, useDualstackEndpoint: n, useFipsEndpoint: o } = e, s = r != null ? () => f(null, null, function* () { return Nt(yield $(r)()); }) : void 0, a = Object.assign(e, { endpoint: s, tls: t, isCustomEndpoint: !!r, useDualstackEndpoint: $(n ?? !1), useFipsEndpoint: $(o ?? !1) }), c; return a.serviceConfiguredEndpoint = () => f(null, null, function* () { return e.serviceId && !c && (c = Ot(e.serviceId)), c; }), a; };
var q = (function (e) { return e.STANDARD = "standard", e.ADAPTIVE = "adaptive", e; })(q || {}), he = 3, Xo = q.STANDARD;
var On = ["BandwidthLimitExceeded", "EC2ThrottledException", "LimitExceededException", "PriorRequestNotComplete", "ProvisionedThroughputExceededException", "RequestLimitExceeded", "RequestThrottled", "RequestThrottledException", "SlowDown", "ThrottledException", "Throttling", "ThrottlingException", "TooManyRequestsException", "TransactionInProgressException"], Nn = ["TimeoutError", "RequestTimeout", "RequestTimeoutException"], Mn = [500, 502, 503, 504], Dn = ["ECONNRESET", "ECONNREFUSED", "EPIPE", "ETIMEDOUT"], vn = ["EHOSTUNREACH", "ENETUNREACH", "ENOTFOUND"];
var Qo = e => e?.$retryable !== void 0;
var Jo = e => e.$metadata?.clockSkewCorrected, Zo = e => { let t = new Set(["Failed to fetch", "NetworkError when attempting to fetch resource", "The Internet connection appears to be offline", "Load failed", "Network request failed"]); return e && e instanceof TypeError ? t.has(e.message) : !1; }, Fe = e => e.$metadata?.httpStatusCode === 429 || On.includes(e.name) || e.$retryable?.throttling == !0, ze = (e, t = 0) => Qo(e) || Jo(e) || Nn.includes(e.name) || Dn.includes(e?.code || "") || vn.includes(e?.code || "") || Mn.includes(e.$metadata?.httpStatusCode || 0) || Zo(e) || e.cause !== void 0 && t <= 10 && ze(e.cause, t + 1), Pn = e => { if (e.$metadata?.httpStatusCode !== void 0) {
    let t = e.$metadata.httpStatusCode;
    return 500 <= t && t <= 599 && !ze(e);
} return !1; };
var Be = class e {
    ...
    updateClientSendingRate(t) { let r; if (this.updateMeasuredRate(), Fe(t)) {
...
var $n = "amz-sdk-invocation-id", kn = "amz-sdk-request";
var Ln = () => { let e = 100; return { computeNextBackoffDelay: n => Math.floor(Math.min(2e4, Math.random() * 2 ** n * e)), setDelayBase: n => { e = n; } }; };
var Ct = ({ retryDelay: e, retryCount: t, retryCost: r }) => ({ getRetryCount: () => t, getRetryDelay: () => Math.min(2e4, e), getRetryCost: () => r });
var te = class {
    maxAttempts;
    mode = q.STANDARD;
    capacity = 500;
    retryBackoffStrategy = Ln();
    maxAttemptsProvider;
    constructor(t) { this.maxAttempts = t, this.maxAttemptsProvider = typeof t == "function" ? t : () => f(this, null, function* () { return t; }); }
    acquireInitialRetryToken(t) { return f(this, null, function* () { return Ct({ retryDelay: 100, retryCount: 0 }); }); }
    refreshRetryTokenForRetry(t, r) { return f(this, null, function* () { let n = yield this.getMaxAttempts(); if (this.shouldRetry(t, r, n)) {
        ...
        return this.capacity -= a, Ct({ retryDelay: i, retryCount: t.getRetryCount() + 1, retryCost: a });
...
var je = class {
    ...
    mode = q.ADAPTIVE;
    constructor(t, r) { this.maxAttemptsProvider = t; let { rateLimiter: n } = r ?? {}; this.rateLimiter = n ?? new Be, this.standardRetryStrategy = new te(t); }
...
var Pp = e => { let { retryStrategy: t, retryMode: r, maxAttempts: n } = e, o = $(n ?? he); return Object.assign(e, { maxAttempts: o, retryStrategy: () => f(null, null, function* () { return t || ((yield $(r)()) === q.ADAPTIVE ? new je(o) : new te(o)); }) }); };
var Hn = e => e?.body instanceof ReadableStream;
var Fn = e => e instanceof Error ? e : e instanceof Object ? Object.assign(new Error, e) : typeof e == "string" ? new Error(e) : new Error(`AWS SDK error wrapper for ${e}`);
var rs = e => (t, r) => n => f(null, null, function* () { let o = yield e.retryStrategy(), s = yield e.maxAttempts(); if (ns(o)) {
    o = o;
    let i = yield o.acquireInitialRetryToken(r.partition_id), a = new Error, c = 0, u = 0, { request: p } = n, d = A.isInstance(p);
    for (d && (p.headers[$n] = ot());;)
        try {
            d && (p.headers[kn] = `attempt=${c + 1}; max=${s}`);
            ...
            let m = os(l);
            if (a = Fn(l), d && Hn(p))
                throw (r.logger instanceof ke ? console : r.logger)?.warn("An error was encountered in a non-retryable streaming request."), a;
            try {
    ...
    return o = o, o?.mode && (r.userAgent = [...r.userAgent || [], ["cfg/retry-mode", o.mode]]), o.retry(t, n); }), ns = e => typeof e.acquireInitialRetryToken < "u" && typeof e.refreshRetryTokenForRetry < "u" && typeof e.recordSuccess < "u", os = e => { let t = { error: e, errorType: ss(e) }, r = as(e.$response); return r && (t.retryAfterHint = r), t; }, ss = e => Fe(e) ? "THROTTLING" : ze(e) ? "TRANSIENT" : Pn(e) ? "SERVER_ERROR" : "CLIENT_ERROR", is = { name: "retryMiddleware", tags: ["RETRY"], step: "finalizeRequest", priority: "high", override: !0 }, Vp = e => ({ applyToStack: t => { t.add(rs(e), is); } }), as = e => { if (!I.isInstance(e))
...
var Kp = !1;
var Qp = !1;
var Zp = ({ serviceId: e, clientVersion: t }) => r => f(null, null, function* () { let n = typeof window < "u" ? window.navigator : void 0, o = n?.userAgent ?? "", s = n?.userAgentData?.platform ?? zn.os(o) ?? "other", i = void 0, a = n?.userAgentData?.brands ?? [], c = a[a.length - 1], u = c?.brand ?? zn.browser(o) ?? "unknown", p = c?.version ?? "unknown", d = [["aws-sdk-js", t], ["ua", "2.1"], [`os/${s}`, i], ["lang/js"], ["md/browser", `${u}_${p}`]]; e && d.push([`api/${e}`, t]); let l = yield r?.userAgentAppId?.(); return l && d.push([`app/${l}`]), d; }), zn = { os(e) { if (/iPhone|iPad|iPod/.test(e))
        ...
        return "Safari"; } };
var rm = e => () => Promise.reject(e);
var Bn = ["in-region", "cross-region", "mobile", "standard", "legacy"];
var am = ({ defaultsMode: e } = {}) => Fr(() => f(null, null, function* () { let t = typeof e == "function" ? yield e() : e; switch (t?.toLowerCase()) {
    case "auto": return Promise.resolve(cs() ? "mobile" : "standard");
    case "mobile":
    case "in-region":
    case "cross-region":
    case "standard":
    ...
    default: throw new Error(`Invalid parameter for "defaultsMode", expect ${Bn.join(", ")}, got ${t}`);
} })), cs = () => { let e = window?.navigator; if (e?.connection) {
...
var dm = e => ({ setRegion(t) { e.region = t; }, region() { return e.region; } }), fm = e => ({ region: e.region() });
var us = {};
function pm() { return typeof window < "u" ? window : typeof self < "u" ? self : us; }
export { Rs as a, Ts as b, A as c, I as d, ii as e, ui as f, yi as g, P as h, $ as i, bi as j, to as k, Di as l, no as m, Re as n, H as o, Wt as p, Xt as q, ca as r, Jt as s, O as t, Ze as u, Ee as v, js as w, V as x, M as y, ie as z, ya as A, Ea as B, wa as C, xa as D, ot as E, J as F, ar as G, be as H, cr as I, ka as J, Bt as K, dr as L, Za as M, mr as N, de as O, Ou as P, Lo as Q, D as R, le as S, pt as T, Md as U, nn as V, me as W, vf as X, jf as Y, Pt as Z, We as _, rc as $, pr as aa, Wf as ba, el as ca, tl as da, ke as ea, hn as fa, gn as ga, dl as ha, fl as ia, yn as ja, gl as ka, Il as la, Kp as ma, Qp as na, Hl as oa, jl as pa, Zl as qa, op as ra, he as sa, Xo as ta, Pp as ua, Vp as va, pm as wa, Zp as xa, rm as ya, am as za, dm as Aa, fm as Ba };
